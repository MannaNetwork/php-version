...
{
var output = '<form action=""><select name="subLoc1" onchange="showSubLoc1(this.value,'+nextLevel+','+cat_id+',\''+type+'\',\''+agent_url+'\',\''+agent_folder+'\'),updategoButton(';
...
if(type=="regions"){
output += '\'false\',this.value,\''+myarr[1]+'\',\''+cat_id+'\')"><option value="">' + wording_ajax_regional_menu1 + '</option>';
}
else
{
output += 'this.value,\'false\',\''+myarr[1]+' \',\''+cat_id+'\')"><option value="">' + wording_ajax_menu1 + '</option>';
}
...
{
xmlhttp.open("GET","/php/getsubloc1.php?q="+myarr[1]+"&type=categories&agent_url="+agent_url+"&agent_folder="+agent_folder);
}
  xmlhttp.send();
}
...
var combo_list = JSON.parse(data);
var output = '<form action=""><table id=\'mn_location_table\'><tr><td><select name=\'regional_menu\' onchange="showSubLoc1(this.value, 1, \''+original_cat_id+'\' ,\'categories\'),updategoButton(this.value,\'false\',\''+original_cat_id+'\')"><option value=""> '+wording_ajax_menu1+' </option>';
for ( j = 0; j < combo_list.length; j++) {
		if ( '' !== combo_list[j].name ) {
			if ( combo_list[j].lft + 1 < combo_list[j].rgt ) {
			//y or n tells the AJAX functions whether there are any more subcategories
...
xmlhttp.open("GET","/php/getsubcat1.php?q="+original_cat_id+"&type=categories");

   xmlhttp.send();
}


function updategoButton(submitted_category, submitted_regional_num, original_cat_id)
{
...
var inserturl = '<a href="?gocat=' + mycatarr[1] + '&tregional_num=0' + '"><div class="data-container" style="display: flex; align-items: center; justify-content: center;"><span class="mn_btn" ><img height="50" width="50" src="/php/images/go-button-animation.gif"></span></div></a><div style="width:50px; margin:auto;"><a href="?get_filters_info=true" target="_blank" onClick="window.open(\'?get_filters_info=true\',\'pagename\',\'resizable,height=600,width=800\'); return false;"><img height="42" width="42" src="/php/images/green_arrow.png"></a></div>';
}
else
{
...
var inserturl = '<a href="?gocat=' + mycatarr[1] + '&tregional_num=' + myregarr[1] + '"><button><span class="mn_btn"><img height="50" width="50" src="/php/images/go-button-animation.gif"</span></button></a><div style="width:300px; margin:auto;"><div style="width:50px; margin:auto;"><a href="?get_filters_info=true" target="_blank" onClick="window.open(\'?get_filters_info=true\',\'pagename\',\'resizable,height=600,width=800\'); return false;"><img height="42" width="42" src="/php/images/green_arrow.png"></a></div>'; 
}
}
else
{ 
...
//	window.alert('in mn_ajax.js myregarr[1] = '+myregarr);
	var inserturl = '<a href="?' + twoargumentssplit[0] + '&tregional_num=' + myregarr[1] +'"><button><span class="mn_btn"><img height="50" width="50" src="/php/images/go-button-animation.gif"</span></button></a>';
	}
else
{
var mycatarr = submitted_category.split(":");
var inserturl = '<a  href="?gocat=' + mycatarr[1] + '&' + twoargumentssplit[1] +'"><button><span class="mn_btn"><img height="50" width="50" src="/php/images/go-button-animation.gif"</span></button></a>';
}
}
  document.getElementById("goLink").innerHTML=inserturl; 
    }

function getLinksAllInOne(page_num, category_id,tregional_num, newnum_of_pages, number_of_links, lft, rgt, mn_agent_url, mn_agent_folder){
...
xmlhttp.open("GET","/php/getLinksAllInOne.php?page_num="+page_num+"&category_id="+category_id+"&tregional_num="+tregional_num+"&newnum_of_pages="+newnum_of_pages+"&number_of_links="+number_of_links+"&lft="+lft+"&rgt="+rgt+"&mn_agent_url="+mn_agent_url+"& mn_agent_folder="+mn_agent_folder,true);
...
function loadDoc() 
{
  ...
  xmlhttp.open("GET","/php/ajax_info.txt", true);
    xhttp.send();
}
</script>
