...
<select name="tregional_num" onchange="updategoButton(\'false\',this.value,\'' . htmlspecialchars( $category_id ) . '\'), showSubLoc1(this.value,1,\'' . htmlspecialchars( $category_id ) . '\',\'regions\',\''.htmlspecialchars( $mn_agent_url ) .'\',\''.htmlspecialchars( $mn_agent_folder ) . '\')"><option value="">' . htmlspecialchars( WORDING_AJAX_REGIONAL_MENU1 ) . '</option> ';
//note - the last argument in the call to showSubLoc1 is the current level (i.e. '1") and indicates it is the first sublevel
		...
		echo '</select><br>
		      <div id="locHint1"><b>' . htmlspecialchars( WORDING_AJAX_REGIONAL_MENU1 ) . '</b></div>
